...
describe('team-up requests — rules are evaluated at the moment students act', () => {
  ...
  test('three new-to-QUT students cannot team up when the unit caps at 2', () => {
  ...
  test('the same three can team up in a unit that sets no cap', () => {
    const unitId = h.createUnit(teacher);                                 // cap off
    const [a, b, c] = [1, 2, 3].map(() => h.enrolStudent(unitId, { slots: 'Tue 10', newToQut: 1 }));
    assert.equal(h.teamUp(unitId, a, b), 'approved');
    assert.equal(h.teamUp(unitId, c, a), 'approved');
