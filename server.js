...
db.connect().then(() => {
  // ── Composition root lives in services/container.js so the tests can build
  //    the same graph against a throwaway database. ──
  ...
  // Periodic sweep: resolve any open team-up requests whose expires_at has passed,
  // then produce time-based notifications (deadline buckets) and send email to
  // students who may not have the app open.
  setInterval(() => {
    try { teamRequestService.sweepExpired(); }
    catch (e) { console.error('Request sweep failed:', e); }

    // Team formation is closed once a unit's deadline passes; close out any
    // request still sitting open so nothing shows a countdown it cannot honour.
    try {
      for (const u of db.all(`SELECT unit_id, deadline FROM units`)) {
        if (isLocked(u)) teamRequestService.invalidateOpenForUnit(u.unit_id);
      }
